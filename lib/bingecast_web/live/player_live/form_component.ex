defmodule BingecastWeb.PlayerLive.FormComponent do
  use BingecastWeb, :live_component
  ...
  def update(%{player: player} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(AudioTest.change_player(player))
     end)}
  end

  @impl true
  def handle_event("validate", %{"player" => player_params}, socket) do
    changeset = AudioTest.change_player(socket.assigns.player, player_params)
  ...
  defp save_player(socket, :edit, player_params) do
    case AudioTest.update_player(socket.assigns.player, player_params) do
      {:ok, player} ->
  ...
  defp save_player(socket, :new, player_params) do
    case AudioTest.create_player(player_params) do
      {:ok, player} ->
        notify_parent({:saved, player})
